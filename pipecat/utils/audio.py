...
def ulaw_8000_to_pcm_8000(ulaw_8000_bytes):
    # Convert μ-law to PCM
    pcm_8000_bytes = audioop.ulaw2lin(ulaw_8000_bytes, 2)

    return pcm_8000_bytes
...
# used for exotel, it is essentially just converting from 16kHz to 8kHz
def pcm_16000_to_slin_8000(pcm_16000_bytes):
    # Convert from 16kHz to 8kHz
    slin_8000_bytes = audioop.ratecv(pcm_16000_bytes, 2, 1, 16000, 8000, None)[0]

    return slin_8000_bytes

# used for exotel, it is essentially just converting from 8 kHz to 16kHz
def slin_8000_to_pcm_16000(slin_8000_bytes):
    # Resample from 8000 Hz to 16000 Hz
    pcm_16000_bytes = audioop.ratecv(slin_8000_bytes, 2, 1, 8000, 16000, None)[0]
